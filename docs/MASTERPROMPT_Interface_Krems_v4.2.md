# Masterprompt: Interface Stadtarchiv Krems – Neuarchitektur (v4.2, eingefroren)
...
**Änderungen gegenüber v4.1 (drei Korrekturen nach Etappe 4):** (1) Foto-Ordner-Ermittlung läuft über eine vorab generierte `data/foto_manifest.json` statt über Live-Verzeichnislisting des Webservers (GitHub-Pages-kompatibel). (2) `datePrecision.js` folgt der vollständigen, bereits in der Bestandsaufnahme dokumentierten Mustertabelle inkl. der Korrektur "leer → `undatiert`" statt "leer → `year`". (3) Format von `unsicherheit_anmerkung` bei mehreren Feldern präzisiert (`<feldname>: <Erklärung>`, Pipe-getrennt) für zuverlässiges strukturiertes Parsen statt Text-Heuristik.
