# Masterprompt: Interface Stadtarchiv Krems – Neuarchitektur (v4.4, eingefroren)
...
**Änderungen gegenüber v4.3 (Ergänzung nach Etappe 6, Teil A):** `js/config/archivalienRegistry.js` in die Ordnerstruktur (Abschnitt 3) nachgetragen – technisches Verzeichnis der Archivalientypen/Ansichten/Modul-Pfade, verhindert Duplizierung dieser Information über Kachelauswahl, Ansicht-wechseln-Button und `app.js` hinweg.
...
Farben/Layout-Konstanten laufen ausschließlich über den direkten Import aus `config/constants.js` (z. B. `CAT_COLORS`), nicht über `options` – ein `theme`-Options-Feld existiert bewusst nicht, um keine zwei parallelen Wege für dieselbe Information zu haben.
...
## 5b. Vertrag: Tooltip (`utils/tooltip.js`)

Eine einzige gemeinsame Tooltip-Komponente für alle Visualisierungen – keine visualisierungseigenen Tooltip-Lösungen (weder native SVG-`<title>`-Elemente noch individuelle HTML-Overlays).

**Exportierte Funktionen:**
- `zeigeTooltip(inhalt, ankerElement, containerElement)` – zeigt den Tooltip an, positioniert relativ zu `ankerElement`, mit Rand-Clamping gegen `containerElement` (fällt auf `window` zurück, falls kein Container übergeben wird)
- `versteckeTooltip()` – blendet den Tooltip aus

**Barrierefreiheit ist nicht optional:** Der Tooltip wird sowohl bei `mouseenter`/`mouseleave` als auch bei `focus`/`blur` des Anker-Elements ausgelöst – reine Maus-Events reichen nicht, sonst ist der Tooltip für Tastaturnutzer*innen unerreichbar. Der Inhalt wird zusätzlich über `aria-describedby` mit dem Anker-Element verknüpft, damit Screenreader ihn unabhängig vom sichtbaren Tooltip vorlesen können.

**Rand-Clamping:** Der Tooltip darf nie über den Rand von `containerElement`/Viewport hinausragen – bei drohendem Überlauf klappt er automatisch zur gegenüberliegenden Seite.
...
- Der Hover-Tooltip zeigt `erschliessungsstatus`, die Liste der konkret als unsicher gekennzeichneten Felder dieses Eintrags, sowie den Inhalt von `unsicherheit_anmerkung` (falls vorhanden) an – so lässt sich gezielt genau das unsichere Feld in der jeweiligen Visualisierung hervorheben (z. B. nur der Ort auf der Karte gestrichelt, während der Personenname normal dargestellt wird), statt den ganzen Eintrag pauschal als unsicher zu markieren. Bei mehreren unsicheren Feldern in derselben Zeile werden die einzelnen Erklärungen mit Pipe `|` getrennt (folgt der allgemeinen Pipe-Konvention aus Abschnitt 12, der DataLoader zerlegt das automatisch in eine Liste). **Verbindliches Format pro Listeneintrag: `<feldname>: <Erklärung>`** – der Feldname vor dem ersten Doppelpunkt muss (case-insensitiv) einem echten Feldnamen entsprechen (z. B. `orte: ...` oder `Orte: ...`). Damit lässt sich beim Anzeigen strukturiert filtern, welche Erklärung zu welchem konkreten Feld gehört (Text vor dem ersten `:` abschneiden, mit Feldnamen abgleichen) – das ist strukturiertes Parsen eines festen Formats, keine Heuristik über freien Satzinhalt, und braucht keine spaltenspezifische Sonderbehandlung im DataLoader selbst. Das allgemeine `anmerkung`-Feld (falls vorhanden) wird NICHT im Unsicherheits-Tooltip angezeigt, um Vermischung mit unrelated archivarischen Notizen zu vermeiden.
...
- **Datumsangaben:** Freitext wie überliefert, Datierungsgenauigkeit automatisch aus dem Textformat abgeleitet. Verbindliche Mustertabelle für `datePrecision.js` (übernommen aus der ursprünglichen Bestandsaufnahme, mit einer Korrektur):
...
**Korrektur gegenüber der alten Implementierung:** Ein leeres Datumsfeld ergibt `undatiert`, nicht `year` – der alte Code behandelte fehlende Daten fälschlich wie ein bloßes Jahr, wodurch Einträge ohne jede Datumsangabe teils unbemerkt aus der Zeitachse verschwanden. Ein `undatiert`-Eintrag wird sichtbar dargestellt (z. B. eigener Bereich, wie beim Gantt-Diagramm bereits für fehlende Zeiträume umgesetzt), nicht ausgeblendet.

**Allgemeine Regel für Diagrammtypen ohne räumlich trennbaren Bereich:** Bei Visualisierungen mit kontinuierlicher Zeitachse (Zeitachse, Gantt) bekommt "undatiert"/"nicht zuordenbar" einen eigenen, räumlich abgesetzten Bereich. Bei Diagrammtypen, wo das strukturell nicht möglich ist (z. B. Marimekko-Spalten, Alluvial-Knoten, die zwingend eine feste Kategorie brauchen), wird "undatiert" stattdessen als eigene, klar beschriftete Kategorie/Spalte/Knoten unter den übrigen behandelt. Die Kernregel bleibt in beiden Fällen gleich: sichtbar, nie stillschweigend ausgeblendet – nur die technische Umsetzung passt sich der jeweiligen Diagrammform an.
