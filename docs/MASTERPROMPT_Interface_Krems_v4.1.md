# Masterprompt: Interface Stadtarchiv Krems – Neuarchitektur (v4.1, eingefroren)
...
**Änderungen gegenüber v4.0 (Konkretisierung nach Code-Review der Treemap, Etappe 2):** `utils/tooltip.js` erhält einen konkreten, verbindlichen Funktionsvertrag (bisher nur Stichwort ohne Schnittstelle) – native SVG-`<title>`-Tooltips werden durch die gemeinsame Komponente ersetzt, mit Rand-Clamping UND vollständiger Tastatur-/Screenreader-Zugänglichkeit. `options.theme` als ungenutztes Options-Feld entfernt (Farben laufen weiterhin zentral über den `CAT_COLORS`-Import).
